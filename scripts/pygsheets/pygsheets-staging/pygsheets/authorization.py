...
def _get_user_authentication_credentials(client_secret_file, scopes, credential_directory=None):
    ...
    global authInput
    inputWindow = Tk()
    Label(inputWindow,text="Your web browser should have opened.\n\nFollow Google's instructions there to get your authorisation code.\n\nOnce you have it, paste it into the box below and press the 'Authorise' button").pack()
    authInput = Entry(inputWindow)
    authInput.pack()
    Button(inputWindow,text="Authorise",command=lambda:[getEntry(),inputWindow.destroy()]).pack()
